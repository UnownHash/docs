...
<Callout type="info" emoji="ℹ️">
  ...
  Remember that Flygon-Admin is two parts so you need to build the client when updating or making changes to the frontend code!
...
Since each service is an individual component it is possible to update just one.
