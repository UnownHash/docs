...
<Callout type="warning" emoji="⚠️">
  Dragonite is not public yet!

  If you wish to continue the setup process you can skip the Dragonite install, build, and run steps. The [Hybrid Setup](/docs/other/hybrid-setup) will allow you to continue using RealDeviceMap (RDM) to control your devices while sending data to Golbat for processing.
</Callout>
...
1. Clone repo
    ...
    ```sh
    git clone https://github.com/UnownHash/Dragonite.git
    ```
...
## Install Dragonite-Admin

1. Clone repo

    ```sh
    git clone https://github.com/UnownHash/Dragonite-Admin.git
    ```
...
<Callout type="info" emoji="ℹ️">
    Skip `yarn start` if you plan to run Dragonite-Admin in PM2
</Callout>
...
1. Navigate to each service folder and compile a static binary
    ...
    cd /path/to/Dragonite && go build Dragonite
    cd /path/to/Dragonite-Admin/server && go build
    ```
...
1. Run the following to start and save a service
   ...
   ```sh
    ...
    > Replace 'golbat' with 'Dragonite' or 'server' (for Dragonite-Admin)
    ...
    module.exports = {
      apps : [
        ...
        },
        {
            ...
            script: 'server',
            cwd: '/home/username/Dragonite-Admin/dist/server',
            instances: 1,
            autorestart: true,
            log_date_format: "YYYY-MM-DD HH:mm",
...
Since each service is an individual component it is possible to update just one. IE - no need to stop our data parser (Golbat) because the device controller (Dragonite) has an update.

> Below example is for Dragonite but Golbat & Dragonite-Admin are the exact same. Just remember to build your client for Dragonite-Admin!
...
1. Copy the below script
    ...
    echo "Updating Dragonite-Admin"
    cd "$START_DIR"/Dragonite-Admin/
    git pull
    yarn install
    yarn build

    cd ~
    pm2 restart golbat Dragonite Dragonite-admin
