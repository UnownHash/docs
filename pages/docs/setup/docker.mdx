...
<Callout type="warning" emoji="⚠️">
  ...
  If you wish to continue the setup process make sure to comment out or delete the Dragonite service. The [Hybrid Setup](/docs/other/hybrid-setup) will allow you to continue using RealDeviceMap (RDM) to control your devices while sending data to Golbat for processing.
</Callout>
...
1. Download the docker-compose example file
    ...
    services:
      Dragonite:
          image: ghcr.io/unownhash/Dragonite:main
      ...
      golbat:
          ...
          #  - "host.docker.internal:host-gateway"
      admin:
          image: ghcr.io/unownhash/Dragonite-admin:main
          ...
          environment:
            ...
            ADMIN_DRAGONITE_API_ENDPOINT: http://dragonite:9002
...
1. Download Dragonite config file
    ...
    ```sh
    wget -O Dragonite_config.toml https://raw.githubusercontent.com/UnownHash/Dragonite/main/config.toml.example
    ```
...
1. Modify your two config files and docker-compose files
    ...
    ```toml filename="Dragonite_config.toml" {2,4,7-11}
    [processors]
